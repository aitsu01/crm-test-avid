...
const props = defineProps<{
    ...
    shouldIncludeSelectionInUrl?: boolean;
    showLabel?: boolean;
    disabled?: boolean;
...
const fullUrl = computed(() => {
    ...
    if (props.shouldIncludeSelectionInUrl) {
        const separator = props.url.includes('?') ? '&' : '?';
        return `${props.url}${separator}selection=${props.selection.join(',')}`;
    }
...
const runAction = () => {
    ...
    router.visit(fullUrl.value, {
        method: props.method,
        data: {
            selection: props.method !== 'get' ? props.selection : undefined,
        },
        preserveScroll: true,
        preserveState: true,
    });
...
<template>
    <div>
        <component :is="wrapper" :url="fullUrl" :modal-size="modalSize">
            <PrimeButton v-tooltip.top="showLabel ? (label ?? undefined) : label" :icon="icon" :aria-label="label"
                :label="showLabel ? label : undefined" :severity="severity" :disabled="disabled" @click="onClick" />
        </component>
    </div>
</template>
